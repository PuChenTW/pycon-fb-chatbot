...
class Messenger(BaseMessenger):
    ...
    def postback(self, message):
        ...
        if "start" in payload:
            txt = (
                "Hey, let's get started! Try sending me one of these messages: "
    ...
    def init_bot(self):
        ...
        persistent_menu = PersistentMenu(menu_items=[menu_item_1, menu_item_2])
...
def webhook():
    if request.method == "GET":
        ...
        try:
            messenger.send(
                {"text": reply}, "RESPONSE", notification_type="REGULAR", timeout=4
            )
        except Exception as e:
            print(e)
            messenger.send(
                {"text": "我不知道"}, "RESPONSE", notification_type="REGULAR", timeout=4
            )
    return ""
...
def _get_result_from_dialogflow(text_to_be_analyzed: str) -> str:
    ...
    try:
        ...
        raise
    return "\n".join(
        fulfillment_message.text.text[0]
